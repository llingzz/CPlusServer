...
bool CIocpServer::OnRequest(void* lpParam1, void* lpParam2)
{
	...
	SAFE_DELETE_ARRAY(lpRequest->m_pDataPtr);
	return true;
}
...
bool CIocpServer::BeginBindListen(const char* lpSzIp, UINT nPort, UINT nInitAccepts, UINT nMaxAccpets)
{
	...
	{
		myLogConsoleE("%s m_lpfnAcceptEx为nullptr!!!", __FUNCTION__, m_pListenContext->m_hSocket);
		return false;
	}
	...
	{
		myLogConsoleE("%s m_lpfnGetAcceptExSockaddrs为nullptr!!!", __FUNCTION__, m_pListenContext->m_hSocket);
		return false;
	}
...
bool CIocpServer::OnReceiveData(DWORD dwKey, CSocketBuffer* pBuffer, DWORD dwTrans)
{
	...
	while (uiDataBufLen > uiPacketHeadLen)
	{
		...
		if (lpPacketHead && lpPacketHead->uiPacketLen <= (uiDataBufLen - uiPacketHeadLen))
		{
			...
			{
				myLogConsoleW("%s 数据包头校验失败", __FUNCTION__);
				bClose = true;
				break;
			}
...
bool CIocpServer::OnHandleData(LPPACKET_HEAD lpPacketHead, DWORD dwKey, DWORD dwTrans)
{
	...
	lpRequest->m_pDataPtr = new BYTE[uiPacketHeadLen + lpPacketHead->uiPacketLen];
	lpRequest->m_stHead = { 0 };
	lpRequest->m_nDataLen = lpPacketHead->uiPacketLen;
	pContext->m_objDataBuf.Read((PBYTE)(lpRequest->m_pDataPtr), uiPacketHeadLen + lpPacketHead->uiPacketLen);
...
void CIocpServer::ReleaseSocketContext(CSocketContext* pContext)
{
	...
	pContext->m_nCurrentReadSerialNo = 0;
	pContext->m_nNextReadSerialNo = 0;
	pContext->m_nPacketNo = 0;
	pContext->m_nSessionID = 0;
	pContext->m_bClose = false;
	pContext->m_objDataBuf.ClearBuffer();
	pContext->m_objReqBuf.ClearBuffer();
	pContext->m_objResBuf.ClearBuffer();
	/*将释放掉的套接字添加到空闲列表重新利用*/
	CAutoLock lock(&m_csSocketContextList);
...
void CIocpServer::FreeSocketContext()
{
	...
	CSocketContext* pNext = nullptr;
	while (pTemp)
	{
		pNext = pTemp->m_pNext;
		pTemp->m_objDataBuf.ClearBuffer();
		pTemp->m_objReqBuf.ClearBuffer();
		pTemp->m_objResBuf.ClearBuffer();
...
bool CIocpClient::Create(const char* lpSzIp, UINT nPort, UINT nMaxSocketBufferListCount, UINT nMaxSocketContextListCount, UINT nMaxSendCount, UINT nThreads, UINT nConcurrency, UINT nMaxConnections)
{
	InitializeMembers();
...
bool CIocpClient::BeginConnect(const char* lpSzIp, UINT nPort)
{
	...
	{
		myLogConsoleW("%s 套接字%d初始化失败", __FUNCTION__, hSocket);
		return false;
	}
...
SOCKET CIocpClient::GetSocket()
{
	CAutoLock lock(&m_csConnectionContext);
	if (m_pConnectionContext)
	{
		return m_pConnectionContext->m_hSocket;
	}
	return INVALID_SOCKET;
}

bool CIocpClient::SendBufferData(void* pDataPtr, int nDataLen)
{
	SOCKET hSocket = GetSocket();
	UINT uiMsgType = 1;
	return SendData(GetSocket(), pDataPtr, nDataLen, uiMsgType);
}